...
요구사항 텍스트를 입력하면 로컬 Claude CLI가 자동으로 4단계 스펙 문서를 생성한다.
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| POST | `/api/run/{profile}` | 스테이지 실행 (intake/spec/jira/qa) |
...
```json
{
  "Cli": { "Command": "claude", "Args": "-p -", "TimeoutSeconds": 120 },
